...
export type CountryCodeType = (typeof countryCodes)[number];

export const countryNames = countries.map((country) => country.name);
export const CountryNameSchema = Type.Union(
  countryNames.map((name) => Type.Literal(name))
);
export type CountryNameType = (typeof countryNames)[number];
